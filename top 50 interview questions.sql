...
select distinct SALARY, DEPARTMENT, FIRST_NAME from worker w1 where 3 > (select count(distinct SALARY) from worker w2 where w1.SALARY < w2.SALARY) order by w1.SALARY desc;
-- Q-47. Write an SQL query to fetch three min salaries from a table using co-related subquery.
select distinct SALARY DEPARTMENT, FIRST_NAME from worker w1 where 3 > (select count(distinct SALARY) from worker w2 where w1.SALARY> w2.SALARY) order by w1.SALARY asc;
-- Q-48. Write an SQL query to fetch nth max salaries from a table.
select distinct * from worker order by SALARY desc limit 2, 1;
-- Q-49. Write an SQL query to fetch departments along with the total salaries paid for each of them.
select distinct * from worker w1 where 2 = (select count(SALARY) from worker w2 where w1.SALARY < w2.SALARY);
-- use limit
